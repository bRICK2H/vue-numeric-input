	...
	export default {
		...
		methods: {
			inputValue(e) {
				...
				if (target.selectionStart > before.length) {
					...
					if (!this.isBothDelete) {
						...
						if (afterUp.length > Number(this.decimal)) {
							afterArr.splice(-1, 1)
			...
			parseValue(val) {
				...
				if (!Number(this.decimal)) {
					...
					if (vAfterCommaLen > Number(this.decimal)) {
						const outerValueArr = String(outerValue).split(''),
							outerCommaIndex = String(outerValue).indexOf('.') + 1

						initValue = outerValueArr.splice(0, Number(this.decimal) + outerCommaIndex).join('')
					} else {
		...
		watch: {
			value: {
				immediate: true,
				handler(val) {
					this.initValue = this.parseValue(val)

				}
			}
		},
		created() {
			this.prevValue = this.parseValue(this.value)
		},
	}
</script>
